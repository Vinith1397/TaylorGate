...
import MobileTracker from "../componets/MobileTracker";
...
/* --- Domino’s-style mobile tracker --- */
.mob-tracker { padding: 8px 8px 6px; }
...
/* Filled bar uses scaleX so text reflow never happens */
.mob-fill{
...
.mob-caret{
  ...
  border-left:14px solid #86b837;  /* overridden by inline borderLeftColor */
...
/* Ensure all layout widths respect padding */
*,
*::before,
*::after { box-sizing: border-box; }

/* The wrapper clamps the tracker to the card's content box */
.tracker-wrap{
  width: 100%;             /* never exceed the card’s inner width */
  max-width: 100%;
  overflow: hidden;        /* clip any sub-pixel overflow (ticks/caret) */
  padding: 0 12px;         /* optional side breathing room inside card */
}

/* Make sure flex parents can actually shrink children */
.minw0 { min-width: 0; }

/* Mobile tracker itself must not use 100vw. Use 100% of its wrapper */
.mob-tracker{ width: 100%; max-width: 100%; margin: 0; padding: 8px 0; }

/* The pill track strictly fits its container */
.mob-track{
  position: relative;
  width: 100%;
  height: 20px;
  border-radius: 9999px;
  background: #e8ecef;
  overflow: hidden;              /* keeps fill/ticks/caret inside */
  box-shadow: inset 0 1px 2px rgba(0,0,0,.08);
}

/* Filled bar uses scaleX but the layer itself spans 100% of the track */
.mob-fill{
...
/* Step ticks – clamped by overflow on .mob-track */
.mob-tick{
  position:absolute; top:2px; width:3px; height:16px;
  transform: translateX(-50%);
  border-radius: 2px;
...
/* Caret is also clipped by .mob-track overflow */
.mob-caret{
  position:absolute; top:50%; transform: translate(-50%, -50%);
  width:0; height:0;
  border-top:10px solid transparent;
  border-bottom:10px solid transparent;
  border-left:14px solid #86b837;
  filter: drop-shadow(0 1px 1px rgba(0,0,0,.2));
}

/* Labels wrap line is 100% and scrolls if too tight */
.mob-labels{
  display:flex; gap:8px; margin-top:8px;
  overflow-x:auto; padding-bottom:2px;
  -webkit-overflow-scrolling: touch;
...
.chev-track{
  display:flex;
  width:100%;
  height:28px;                 /* keep your current visual scale */
  overflow:hidden;
}

.chev-seg{
  --seg:#cfd4d9;               /* default color (upcoming) */
  position:relative;
  flex:1 1 0;
  background: var(--seg);
  margin-right:6px;            /* small joint gap like the reference */
}
.chev-seg:last-child{ margin-right:0; }
...
.chev-seg::after{
  ...
  border-left:14px solid var(--seg);
}

/* Left “notch” so it looks ribbon-joined (skip for the first piece) */
.chev-seg:not(.first)::before{
  content:"";
  position:absolute; top:0; left:0;
  border-top:14px solid transparent;
  border-bottom:14px solid transparent;
  border-left:7px solid #fff;   /* white of the card below */
}
...
.chev-seg > .hole{
  ...
  box-shadow: 0 0 0 2px rgba(255,255,255,.25); /* subtle */
}
...
.mob-stepnums .num{
  ...
  font-size:16px; line-height:1;           /* >=16 to avoid iOS zoom */
  color:#9aa39f;
}
.mob-stepnums .num.on{ color:#000; }       /* current */
.mob-stepnums .num.done{ color:#4d6f1d; }  /* completed */
...
`}</style>
      ...
      <div style={{ flex: 1, display: "flex", justifyContent: "center", minHeight: 0, minWidth: 0 }}>
        <Card
          className="card"
          style={{
            width: "100%",
      ...
      >
        ...
        >
          {}
              ...
              <div className="cardScroll minw0">
